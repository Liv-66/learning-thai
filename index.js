...
// Require and use body-parser to parse the request body.
...
app.get('/api/quiz', async (req, res) => {
  ...
  try {
    // Create db connection from utils/db.js
    const db = require('./utils/db.js');
    // Get data from db
    const vocabularies = await db.getVocabularies();

    data.vocabularies = vocabularies;
  } catch (e) {
    console.error(e);
    data.vocabularies = e.message;
  }
  res.status(200).send(data);
});

// Post data to insert into db table: vocabularies
app.post('/api/vocabularies', async (req, res) => {
  try {
    // Create db connection from utils/db.js
    const db = require('./utils/db.js');
    // Get data from db
    await db.insertVocabularies(req.body);
    const vocabularies = await db.getVocabularies();
